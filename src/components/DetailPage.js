...
import { numberWithCommas, baseURL, apiURL, checkAccess } from './Common';
...
function DetailPage({ setSign, setReset }) {
        ...
        <div className="main-frame no-select">
          ...
          <div className="description">
            ...
            <div className="btn-container">
              <button className={item.price < 0 ? "btn-disabled" : ""}
                style={{ "--color": "#FF0000" }}
                onClick={item.price > 0 ? () => {
                  if (!checkAccess()) {
                    setSign(1)
                    return
                  }
                ...
                {item.price === -1 ? "Out of stock" : item.price === -2 ? "Contact for more" : "Buy now"}
              </button>
              {item.price > 0 ? <button style={{ "--color": "#131212" }}
                onClick={() => {
                  if (!checkAccess()) {
                    setSign(1)
                    return
